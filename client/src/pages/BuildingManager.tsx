...
const getRiskBadge = (score: number) => {
  ...
  return <StatusBadge status="danger" pulse>High Risk</StatusBadge>;
};

export const BuildingManager = () => {
  const [selectedBuilding, setSelectedBuilding] = useState<Building | null>(null);
  const [activeTab, setActiveTab] = useState<"details" | "ownership" | "scans">("details");
  const [showAddModal, setShowAddModal] = useState(false);
  const [showTransferModal, setShowTransferModal] = useState(false);
  
  // Logic: "list" view is default on mobile. "detail" view takes over when a building is clicked.
  ...
  const buildingOwnerships = selectedBuilding
    ? mockOwnerships.filter((o) => o.building_id === selectedBuilding.building_id)
    : [];
  ...
  const handleAddBuilding = () => {
    ...
    console.log("Add Building Payload:", JSON.stringify(payload, null, 2));
  ...
  const handleTransferOwnership = () => {
    ...
    console.log("Transfer Ownership Payload:", JSON.stringify(payload, null, 2));
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="md:ml-64 p-4 md:p-8 transition-all duration-300">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-6">
          ...
          <motion.div
            ...
            className={`md:col-span-4 ${mobileView === "detail" ? "hidden md:block" : "block"}`}
          >
            <GlassCard className="h-auto md:h-[calc(100vh-180px)] overflow-hidden" hover={false}>
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-semibold text-foreground">Buildings</h3>
                <span className="text-xs text-muted-foreground">
                  {mockBuildings.length} registered
                </span>
              </div>
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground mt-1 flex items-center gap-1">
                          ...
                          <span className="truncate">{building.address_text}</span>
                        </p>
                        <div className="flex items-center gap-2 mt-2">
                          <span className="text-xs text-muted-foreground">
                            Est. {building.construction_year}
                          </span>
                          <span className={`text-xs font-medium ${getRiskColor(building.risk_score)}`}>
          ...
          <motion.div
            initial={{ opacity: 0, x: 20 }}
            animate={{ opacity: 1, x: 0 }}
            className={`md:col-span-8 ${mobileView === "list" ? "hidden md:block" : "block"}`}
          >
            {selectedBuilding ? (
              <GlassCard className="h-auto md:h-[calc(100vh-180px)] overflow-hidden" hover={false}>
                  ...
                  <AnimatePresence mode="wait">
                    ...
                    {activeTab === "details" && (
                      ...
                      >
                        <div className="p-4 bg-muted/20 rounded-xl">
                          <p className="text-xs text-muted-foreground mb-1">Building ID</p>
                          <p className="font-mono text-sm text-foreground">
                            {selectedBuilding.building_id}
                          </p>
                        </div>
                        <div className="p-4 bg-muted/20 rounded-xl">
                          <p className="text-xs text-muted-foreground mb-1">Construction Year</p>
                        ...
                        <div className="p-4 bg-muted/20 rounded-xl">
                          <p className="text-xs text-muted-foreground mb-1">GPS Latitude</p>
                          <p className="font-mono text-sm text-foreground">
                            {selectedBuilding.location_gps.lat}
                          </p>
                        </div>
                        <div className="p-4 bg-muted/20 rounded-xl">
                          <p className="text-xs text-muted-foreground mb-1">GPS Longitude</p>
                          <p className="font-mono text-sm text-foreground">
                        ...
                        <div className="sm:col-span-2 p-4 bg-muted/20 rounded-xl">
                          ...
                          <div className="flex items-center gap-3">
                            <div className="flex-1 h-3 bg-muted/30 rounded-full overflow-hidden">
                              <motion.div
                                initial={{ width: 0 }}
                                animate={{ width: `${selectedBuilding.risk_score}%` }}
                            ...
                            <span className={`font-bold ${getRiskColor(selectedBuilding.risk_score)}`}>
                    ...
                    {activeTab === "ownership" && (
                      ...
                      >
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
                          <h4 className="font-medium text-foreground">Current Owners</h4>
                          <button
                            ...
                            >
                              ...
                              <div className="mt-3 pt-3 border-t border-white/5 flex flex-wrap gap-4 text-sm">
                                <span className="text-muted-foreground flex items-center gap-1">
                                  ...
                                  Start: {ownership.start_date}
                                </span>
                                {ownership.end_date && (
                                  <span className="text-muted-foreground flex items-center gap-1">
                                    <Calendar className="h-3 w-3" />
                                    End: {ownership.end_date}
                                  </span>
                                )}
                              </div>
                            </div>
                    ...
                    {activeTab === "scans" && (
                      <motion.div
                        ...
                        className="space-y-4"
                      >
                        <h4 className="font-medium text-foreground mb-4">AI Scan History</h4>
                              ...
                              <div className="mt-3 pt-3 border-t border-white/5 grid grid-cols-2 sm:grid-cols-3 gap-3">
                                <div>
                                  ...
                                  <p className={`font-bold ${
                                    ...
                                    {(scan.ai_confidence_score * 100).toFixed(0)}%
                                  </p>
                                </div>
                                <div>
              ...
              <GlassCard className="h-64 md:h-[calc(100vh-180px)] flex items-center justify-center" hover={false}>
      ...
      <Dialog open={showAddModal} onOpenChange={setShowAddModal}>
        <DialogContent className="bg-card/95 backdrop-blur-xl border-white/10 max-w-md mx-4 overflow-y-auto max-h-[85vh]">
          ...
          <div className="space-y-4 mt-4">
            <div>
              <Label htmlFor="building_name" className="text-foreground">Building Name</Label>
              <Input
                id="building_name"
            ...
            <div>
              <Label htmlFor="address_text" className="text-foreground">Address</Label>
              <Input
                id="address_text"
                value={newBuilding.address_text}
                onChange={(e) => setNewBuilding({ ...newBuilding, address_text: e.target.value })}
            ...
            <div>
              <Label htmlFor="construction_year" className="text-foreground">Construction Year</Label>
              <Input
                id="construction_year"
                type="number"
                value={newBuilding.construction_year}
                onChange={(e) => setNewBuilding({ ...newBuilding, construction_year: parseInt(e.target.value) })}
                min={1900}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label htmlFor="location_gps_lat" className="text-foreground">Latitude</Label>
                <Input
                  id="location_gps_lat"
                  type="number"
                  step="0.000001"
                  value={newBuilding.location_gps_lat}
                  onChange={(e) => setNewBuilding({ ...newBuilding, location_gps_lat: parseFloat(e.target.value) })}
                  placeholder="23.8103"
                  className="mt-1.5 bg-muted/30 border-white/10"
                />
              </div>
              <div>
                <Label htmlFor="location_gps_lng" className="text-foreground">Longitude</Label>
                <Input
                  id="location_gps_lng"
                  type="number"
                  step="0.000001"
                  value={newBuilding.location_gps_lng}
                  onChange={(e) => setNewBuilding({ ...newBuilding, location_gps_lng: parseFloat(e.target.value) })}
                  placeholder="90.4125"
                  className="mt-1.5 bg-muted/30 border-white/10"
                />
              </div>
      ...
      <Dialog open={showTransferModal} onOpenChange={setShowTransferModal}>
        <DialogContent className="bg-card/95 backdrop-blur-xl border-white/10 max-w-md mx-4">
          ...
          <div className="space-y-4 mt-4">
            <div>
              <Label htmlFor="owner_id" className="text-foreground">New Owner</Label>
              <Select
                value={transfer.owner_id}
                onValueChange={(value) => setTransfer({ ...transfer, owner_id: value })}
              >
            ...
            <div>
              <Label htmlFor="start_date" className="text-foreground">Transfer Date</Label>
              <Input
                id="start_date"
                type="date"
                value={transfer.start_date}
                onChange={(e) => setTransfer({ ...transfer, start_date: e.target.value })}
                className="mt-1.5 bg-muted/30 border-white/10"
              />
            </div>
            ...
            <div className="flex gap-3 pt-4">
              ...
              <GlowButton variant="accent" onClick={handleTransferOwnership} className="flex-1">
